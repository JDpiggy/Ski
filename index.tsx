...
const C = {
    ...
    SKIER_HEIGHT: 30,
    SKIER_INITIAL_Y: 50,
    SKIER_HORIZONTAL_SPEED: 2.5,
    SKIER_DRIFT_SPEED: 0.6,
    SKIER_VERTICAL_SPEED_BOOST: 0.15,
    SKIER_VERTICAL_SPRING_FORCE: 0.008,
...
function E(tag: string, classList: string | string[] = [], children: string | Node[] = [], styles: Partial<CSSStyleDeclaration> = {}, attributes: Record<string, any> = {}): HTMLElement {
    ...
    if (typeof classList === 'string' && classList) {
        ...
        el.classList.add(...classList.filter(c => c));
    }
...
function createFooter(tip) {
    footerElement = E('footer', [], [
        E('p', [], [
            E('span', [], 'Tip: ', { fontWeight: '700' }),
...
function updateFooterTip() {
    ...
    if (footerElement) {
        ...
        p.innerHTML = '';
        p.appendChild(E('span', [], 'Tip: ', { fontWeight: '700' }));
        p.appendChild(document.createTextNode(C.GAME_TIPS[newIndex]));
    } else {
...
function createPlayerSelectScreen() {
    const buttons = [1, 2, 3, 4].map((count, index) => {
        const btn = E('button', 'player-select-btn', `${count} Player${count > 1 ? 's' : ''}`, {
    ...
    const screen = E('div', [], [
        E('div', [], [
            E('h1', [], "Jaron's Skifree Battle", { fontSize: '3rem', fontWeight: '800', color: 'rgb(30, 41, 59)', marginBottom: '0.5rem', letterSpacing: '-0.025em' }),
            E('p', [], 'Select number of players:', { color: 'rgb(100, 116, 139)', marginBottom: '2rem' }),
            E('div', [], buttons, { display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '1rem' }),
        ], { 
...
function createGameOverScreen() {
    ...
    const winnerTitleText = winner ? `${winnerColorName} Wins!` : 'Game Over';
    const winnerStyles = winner
        ? { color: winner.color }
        : { color: '#dc2626' };
    
    const title = E('h2', [], winnerTitleText, { ...winnerStyles, fontSize: '3rem', fontWeight: '800', marginBottom: '0.5rem' });

    const playAgainBtn = E('button', 'play-again-btn', 'Play Again');
    playAgainBtn.onclick = restartGame;
    
    const changePlayersBtn = E('button', 'change-players-btn', 'Change Players');
    changePlayersBtn.onclick = changePlayers;

    const screen = E('div', [], [
        E('div', [], [
            title,
            E('p', [], winner ? 'The last skier standing!' : 'You wiped out!', { color: 'rgb(100, 116, 139)', marginBottom: '1rem', fontSize: '1.125rem' }),
            E('div', [], [
                E('p', [], 'FINAL DISTANCE', { color: 'rgb(100, 116, 139)', fontSize: '0.875rem', fontWeight: '500' }),
                E('p', [], `${finalScore}m`, { fontSize: '3.75rem', fontWeight: '700', color: 'rgb(30, 41, 59)' }),
            ], { textAlign: 'center', backgroundColor: 'rgb(241, 245, 249)', padding: '1rem', borderRadius: '0.5rem', marginBottom: '2rem' }),
            E('div', [], [playAgainBtn, changePlayersBtn], { display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '1.5rem' }),
        ], {
            backgroundColor: 'white', padding: '2rem', borderRadius: '1rem', boxShadow: '0 25px 50px -12px rgba(0,0,0,.25)', textAlign: 'center', width: '100%', maxWidth: '28rem', borderTop: '4px solid rgb(59,130,246)'
        }),
    ], { 
        width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem', backgroundColor: 'rgba(248, 250, 252, 0.8)', backdropFilter: 'blur(4px)'
    });
    clearApp();
    app.appendChild(screen);
    app.appendChild(footerElement);
...
function createGameScreen() {
    clearApp();
    gameContainer = E('div', [], [], { width: '100%', height: '100%', backgroundColor: 'white', overflow: 'hidden', position: 'relative' });
    gameElementsContainer = E('div', [], [], {
        position: 'absolute',
        top: '0px',
        left: '0px',
    ...
    const scoreDisplay = E('div', [], `DISTANCE: ${score}m`, {
        position: 'absolute', top: '1rem', right: '1rem', backgroundColor: 'rgba(15, 23, 42, 0.7)', color: 'white', fontWeight: '700', fontSize: '1.5rem', padding: '0.5rem 1rem', borderRadius: '0.5rem', zIndex: '10'
    }, { id: 'score-display' });
...
function createEntityElement(entity) {
    const el = E('div', [], [], { position: 'absolute' });
    if (entity.hasOwnProperty('opacity')) { // Particle
        el.style.borderRadius = '9999px';
        el.style.backgroundColor = 'rgb(203, 213, 225)';
    }
    return el;
}
...
function updateEntityElement(el, entity) {
    if (entity.hasOwnProperty('isAlive')) { // Skier
        ...
        const powerUpRing = entity.hasPowerUp ? `<div class="animate-pulse" style="position: absolute; border-radius: 9999px; width: ${C.SKIER_WIDTH + 20}px; height: ${C.SKIER_HEIGHT + 15}px; left: -10px; top: -5px; border: 3px solid ${C.POWERUP_COLOR}; box-shadow: 0 0 8px ${C.POWERUP_COLOR};"></div>` : '';
        el.innerHTML = `${powerUpRing}
            <div style="width: 100%; height: 100%; position: relative; transform: rotate(${rotation}deg); transition: transform 0.1s linear; transform-origin: bottom center;">
                <div style="position: absolute; background-color: rgb(30, 41, 59); width: 4px; height: 100%; left: 2px; top: 0; border-radius: 2px;"></div>
                <div style="position: absolute; background-color: rgb(30, 41, 59); width: 4px; height: 100%; right: 2px; top: 0; border-radius: 2px;"></div>
                <div style="position: absolute; border-top-left-radius: 0.375rem; border-top-right-radius: 0.375rem; width: 10px; height: 18px; left: 50%; transform: translateX(-50%); top: 5px; background-color: ${entity.color};"></div>
            </div>`;
        ...
        // FIX: Explicitly type `styles` to allow adding more style properties.
        const styles: Partial<CSSStyleDeclaration> = { left: `${entity.position.x}px`, top: `${entity.position.y}px`, width: `${entity.size}px`, height: `${entity.size}px` };
        if (entity.type === ObstacleType.Rock) {
            styles.backgroundColor = 'rgb(100, 116, 139)';
        ...
        el.innerHTML = `
            <div style="position: absolute; border-radius: 9999px; top: 10px; left: 8px; width: 8px; height: 8px; background-color: ${entity.targetPlayerColor}; box-shadow: 0 0 5px ${entity.targetPlayerColor};"></div>
            <div style="position: absolute; border-radius: 9999px; top: 10px; right: 8px; width: 8px; height: 8px; background-color: ${entity.targetPlayerColor}; box-shadow: 0 0 5px ${entity.targetPlayerColor};"></div>
            <div style="position: absolute; background-color: rgb(30, 41, 59); bottom: 10px; left: 50%; transform: translateX(-50%); width: 20px; height: 4px; border-radius: 2px;"></div>`;
        Object.assign(el.style, { left: `${entity.position.x}px`, top: `${entity.position.y}px`, width: `${C.YETI_WIDTH}px`, height: `${C.YETI_HEIGHT}px`, backgroundColor: 'rgb(226, 232, 240)', borderRadius: '0.375rem', border: '2px solid #64748b' });
    } else { // PowerUp
        el.style.borderRadius = '9999px';
...
function createReadyUpOverlay() {
    const playerDisplays = players.map(player => E('div', [], [
        E('p', [], 'Press Key', { fontSize: '1.5rem', marginBottom: '0.5rem', opacity: '0.5' }, { id: `ready-status-${player.id}` }),
        E('div', [], player.readyKey, { width: '6rem', height: '6rem', borderRadius: '0.5rem', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '3rem', boxShadow: '0 10px 15px -3px rgba(0,0,0,.1), 0 4px 6px -2px rgba(0,0,0,.05)', border: '4px solid rgba(255, 255, 255, 0.5)', backgroundColor: player.color }),
        E('p', [], `Steer Left: [${player.controlKey}]`, { marginTop: '0.5rem', fontSize: '0.875rem', backgroundColor: 'rgba(0,0,0,0.5)', padding: '0.25rem 0.5rem', borderRadius: '0.25rem' }),
    ], { textAlign: 'center', color: 'white', fontWeight: '700' }));
    
    return E('div', [], [
        E('h2', 'animate-pulse', 'GET READY!', { fontSize: '3.75rem', fontWeight: '700', color: 'white', marginBottom: '2rem' }),
        E('div', [], playerDisplays, { display: 'flex', justifyContent: 'space-around', alignItems: 'flex-end', width: '100%', maxWidth: '56rem', paddingLeft: '1rem', paddingRight: '1rem' })
    ], { 
        position: 'absolute', top: '0', right: '0', bottom: '0', left: '0', backgroundColor: 'rgba(15, 23, 42, 0.8)', zIndex: '20', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center'
    }, { id: 'ready-up-overlay' });
}
